...
export const useDataSync = () => {
      ...
      try {
        ...
        try {
          ...
          if (retryCountRef.current >= MAX_RETRY_ATTEMPTS) {
            console.log(
              "Max retry attempts reached, falling back to local data"
            );
            toast.error(
              "Couldn't load data after multiple attempts. Using local data."
            );
  ...
  const forceRefresh = useCallback(
    async (setGameData: React.Dispatch<React.SetStateAction<GameData>>) => {
      setIsLoading(true);
