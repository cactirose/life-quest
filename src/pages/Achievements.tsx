...
import { useGameData, Achievement, AchievementCategory, GearItem } from "@/contexts/DataContext";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, DialogFooter } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Badge, BadgeCheck, BadgePercent, Award, PlusCircle, Trash2, Edit, Sparkle, Coins, Swords, Medal, BookOpen, UserCircle, BarChart } from "lucide-react";
...
const AchievementCard = ({
  ...
  const progress = achievement.requiredCount && achievement.currentCount !== undefined ? Math.min(100, achievement.currentCount / achievement.requiredCount * 100) : 0;
  const isTrackable = achievement.requiredCount !== undefined && achievement.currentCount !== undefined;
  return <div className={`wood-texture p-4 relative overflow-hidden border-2 ${achievement.unlocked ? "border-rpg-green" : "border-rpg-brown"}`}>
      {achievement.unlocked && <div className="absolute top-0 right-0 bg-rpg-green text-white px-2 py-1 text-xs transform translate-x-2 -translate-y-2 rotate-45">
          Unlocked
        </div>}
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        {!achievement.unlocked && <div className="flex items-center gap-1">
            <Button onClick={() => onEdit(achievement)} variant="outline" size="sm" className="p-1 h-8 w-8">
              <Edit size={14} />
            </Button>
            
            <Button onClick={() => onDelete(achievement.id)} variant="outline" size="sm" className="p-1 h-8 w-8 text-rpg-red hover:text-white hover:bg-rpg-red">
              <Trash2 size={14} />
            </Button>
          </div>}
      </div>
      ...
      {isTrackable && !achievement.unlocked && <div className="mb-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 text-xs text-rpg-brown">
          ...
          {achievement.specialReward && <div className="flex items-center">
              <BadgePercent size={14} className="mr-1 text-rpg-purple" />
              <span className="text-rpg-purple">Special</span>
            </div>}
        </div>
        
        {achievement.unlocked ? <div className="text-xs text-rpg-brown">
            Unlocked: {achievement.dateUnlocked ? format(new Date(achievement.dateUnlocked), "MMM d, yyyy") : "Unknown"}
          </div> : <Button onClick={() => onUnlock(achievement.id)} variant="outline" size="sm" className="bg-rpg-green text-white border-none hover:bg-rpg-light-green">
...
const AchievementForm = ({
  ...
  return <div className="space-y-4">
      <div>
        ...
        <Input id="title" value={title} onChange={e => setTitle(e.target.value)} placeholder="Enter achievement title" className="w-full" />
      </div>
      ...
      <div>
        ...
        <Textarea id="description" value={description} onChange={e => setDescription(e.target.value)} placeholder="Enter achievement description" className="w-full" />
      </div>
      ...
      <div>
        ...
        <Select value={category} onValueChange={(value: AchievementCategory) => setCategory(value)}>
          <SelectTrigger>
      ...
      <div>
        ...
        <div className="grid grid-cols-10 gap-2">
          {emojiOptions.map(emoji => <button key={emoji} type="button" onClick={() => setIcon(emoji)} className={`h-8 w-8 flex items-center justify-center rounded-md border ${icon === emoji ? "border-2 border-rpg-brown bg-rpg-tan" : "border-border hover:bg-accent"}`}>
              {emoji}
            </button>)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input id="xpReward" type="number" min="0" value={xpReward} onChange={e => setXpReward(Number(e.target.value))} className="w-full" />
        </div>
        ...
        <div>
          ...
          <Input id="coinReward" type="number" min="0" value={coinReward} onChange={e => setCoinReward(Number(e.target.value))} className="w-full" />
        </div>
      </div>
      
      <div className="flex items-center space-x-2">
        <input type="checkbox" id="isTrackable" checked={isTrackable} onChange={e => setIsTrackable(e.target.checked)} className="h-4 w-4 rounded" />
      ...
      {isTrackable && <div>
          ...
          <Input id="requiredCount" type="number" min="1" value={requiredCount} onChange={e => setRequiredCount(Number(e.target.value))} className="w-full" />
          <p className="text-xs text-muted-foreground mt-1">
            How many items must be completed to unlock this achievement
          </p>
        </div>}
      
      {initialData?.specialReward && <div className="border p-3 rounded-md bg-rpg-parchment/50">
...
const Achievements = () => {
  ...
  const completionPercentage = achievements.length > 0 ? Math.round(unlockedAchievements.length / achievements.length * 100) : 0;
  ...
  return <div className="container mx-auto animate-fade-in">
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          ...
          <DialogContent className="max-w-md parchment border-none">
            ...
            <AchievementForm onSubmit={handleAddAchievement} onCancel={() => setShowAddDialog(false)} />
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={!!editingAchievement} onOpenChange={open => !open && setEditingAchievement(null)}>
          <DialogContent className="max-w-md parchment border-none">
            ...
            {editingAchievement && <AchievementForm initialData={editingAchievement} onSubmit={handleUpdateAchievement} onCancel={() => setEditingAchievement(null)} />}
          </DialogContent>
        </Dialog>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="w-full mb-6 font-pixel bg-rpg-tan text-rpg-brown border-2 border-rpg-brown">
          <TabsTrigger value="all" className="flex-1 data-[state=active]:bg-rpg-brown data-[state=active]:text-rpg-tan">
            All ({categories.all})
          </TabsTrigger>
          <TabsTrigger value="quests" className="flex-1 data-[state=active]:bg-rpg-brown data-[state=active]:text-rpg-tan">
            Quests ({categories.quests})
          </TabsTrigger>
          <TabsTrigger value="habits" className="flex-1 data-[state=active]:bg-rpg-brown data-[state=active]:text-rpg-tan">
            Habits ({categories.habits})
          </TabsTrigger>
          <TabsTrigger value="skills" className="flex-1 data-[state=active]:bg-rpg-brown data-[state=active]:text-rpg-tan">
            Skills ({categories.skills})
          </TabsTrigger>
          <TabsTrigger value="character" className="flex-1 data-[state=active]:bg-rpg-brown data-[state=active]:text-rpg-tan">
            Character ({categories.character})
          </TabsTrigger>
          <TabsTrigger value="general" className="flex-1 data-[state=active]:bg-rpg-brown data-[state=active]:text-rpg-tan">
            General ({categories.general})
          </TabsTrigger>
        </TabsList>
        
        {Object.keys(categories).map(category => <TabsContent key={category} value={category} className="animate-fade-in">
            {filterAchievementsByCategory(category).length === 0 ? <div className="text-center py-12 parchment">
                ...
                <Button onClick={() => setShowAddDialog(true)} className="pixel-button">
                  <PlusCircle size={16} className="mr-2" />
                  Create Your First Achievement
                </Button>
              </div> : <>
                {lockedAchievements.filter(a => category === "all" || a.category === category).length > 0 && <div className="mb-8">
                    <h3 className="text-xl font-pixel text-rpg-brown mb-4">Locked Achievements</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {lockedAchievements.filter(a => category === "all" || a.category === category).map(achievement => <AchievementCard key={achievement.id} achievement={achievement} onEdit={setEditingAchievement} onDelete={handleDeleteAchievement} onUnlock={handleUnlockAchievement} />)}
                    </div>
                  </div>}
                
                {unlockedAchievements.filter(a => category === "all" || a.category === category).length > 0 && <div>
                    <h3 className="text-xl font-pixel text-rpg-brown mb-4">Unlocked Achievements</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {unlockedAchievements.filter(a => category === "all" || a.category === category).map(achievement => <AchievementCard key={achievement.id} achievement={achievement} onEdit={setEditingAchievement} onDelete={handleDeleteAchievement} onUnlock={handleUnlockAchievement} />)}
                    </div>
                  </div>}
              </>}
          </TabsContent>)}
